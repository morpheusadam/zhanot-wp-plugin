...
class Zhanot_License
{
    ...
    public function admin_menu()
    {
        if (Zhanot_License::is_activated() !== true) {
            $zhanot_submenu_name = __('Register version', $this->text_domain);
        } else {
            $zhanot_submenu_name = '<span style="color: #FF9800">' . __('Orginal Version', $this->text_domain) . '</span>';
        }
        add_submenu_page(
            $this->parent_slug,
            $zhanot_submenu_name,
    ...
    public function menu_content()
    {
        ...
        <div class="wrap wpmlr_wrap" data-show="<?php echo $data_show ?>">
            ...
            <?php if (isset($now) && !empty($now)): ?>
                <p class="zhanot-alert zhanot-alert-success"><?php _e('You already register your license key. to re validate it, you can use this form.', $this->text_domain); ?></p>
                <div class="current_license">
                    <span class="current_label"><?php _e('Your current license:', $this->text_domain); ?></span>
                    <code class="zhanot-validate-installer"><?php echo $starter; ?></code>
                    <div class="action <?php echo ($now->action == 1) ? 'active' : 'inactive'; ?>">
                        <?php if ($now->action == 1): ?>
                            <span class="dashicons dashicons-yes"></span>
                            <?php echo $now->message; ?><?php else: ?>
                            <span class="dashicons dashicons-no-alt"></span>
                            <?php echo $now->message; ?><?php endif; ?>
                    </div>
                    <div class="keys">
            ...
            <?php else: ?>
                <p class="zhanot-alert zhanot-alert-warning"><?php _e('Please activate plugin with your license key to all features work.', $this->text_domain); ?></p>
                <form action="#" method="post" class="register_version_form">
                ...
                jQuery(document).ready(function ($) {
                    ...
                    jQuery(document).on('submit', '.register_version_form', function (event) {
                        ...
                        jQuery.ajax({
                            ...
                            .fail(function (result) {
                                thisEl.find('.result_text').append('<div class="zhanot-alert zhanot-alert-danger"><?php _e('Something goes wrong please try again.', $this->text_domain); ?></div>').slideDown(150)
                            })
                    ...
                    $(document).on('click', '.wpmlr_revalidate', function (event) {
                        ...
                        $.ajax({
                            ...
                            .fail(function (result) {
                                thisEl.find('.check_result .result_text').append('<div class="zhanot-alert zhanot-alert-danger"><?php _e('Something goes wrong please try again.', $this->text_domain); ?></div>').slideDown(150)
                            })
                            .always(function (result) {
                                thisEl.removeClass('waiting');
                                thisEl.find('.button').removeClass('disabled');
    ...
    public function wp_starter()
    {
        ...
        if (empty($starter)) {
            wp_send_json_error('<div class="zhanot-alert zhanot-alert-danger">' . __('Please insert your license code', $this->text_domain) . '</div>');
        }
        ...
        if ($result->status == 'successful') {
            ...
            $output = '<div class="zhanot-alert zhanot-alert-success">' . __('Thanks! Your license activated successfully.', $this->text_domain) . '</div>';
            wp_send_json_success($output);
        } else {
            if (!is_object($result->message)) {
                $output = '<div class="zhanot-alert zhanot-alert-danger">' . $result->message . '</div>';
                wp_send_json_error($output);
            } else {
                foreach ($result->message as $message) {
                    foreach ($message as $msg) {
                        $output .= '<div class="zhanot-alert zhanot-alert-danger">' . $msg . '</div>';
                    }
                }
                wp_send_json_error($output);
            }
    ...
    public function revalidate_starter()
    {
        ...
        if ($result->status == 'successful') {
            ...
            update_option($rand_key, json_encode($how));
            $output = '<div class="zhanot-alert zhanot-alert-danger">' . $how['message'] . '</div>';
            wp_send_json_success($output);
        }
    }
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: Zhanot_License::is_activated
